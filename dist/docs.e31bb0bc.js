...
var key = '4fa592d94426516b6e4d2bdb8f30b2dd'; //openweathermap API key
var tempPara = document.querySelector('.temp');
var unitsCheck = document.querySelector('.unitsCheck');
var humidityPara = document.querySelector('.humidity');
var windPara = document.querySelector('.wind');
var weatherDescPara = document.querySelector('.weather-description');
var city = document.querySelector('.location');
// const tempUnits = document.querySelector('.tempUnits');
var input = document.querySelector('.input-location-name');
...
function getWeatherByCity(location) {
  fetch('https://api.openweathermap.org/data/2.5/weather?q=' + location + '&appid=' + key).then(function (resp) {
    ...
    humidityPara.textContent = "".concat(data.main.humidity, "%");
    windPara.textContent = "".concat(data.wind.speed, " m/s");
    weatherDescPara.textContent = data.weather[0].description;
    date = "".concat(Date().slice(0, 3), ",\n        ").concat(Date().slice(8, 11), "\n        ").concat(Date().slice(4, 8), "\n        ").concat(Date().slice(11, 16));
...
function getWeatherBylatlon(latitude, longitude) {
  fetch("https://api.openweathermap.org/data/2.5/weather?lat=".concat(latitude, "&lon=").concat(longitude, "&appid=").concat(key)).then(function (resp) {
    return resp.json(); //converting data to json
  }).then(function (data) {
    if (!unitsCheck.checked) {
      temp = (data.main.temp - 273.15).toFixed(2);
...
unitsCheck.addEventListener('change', function () {
  if (!unitsCheck.checked) {
